...
개발 화면 오른쪽 아래의 `데모 덱`은 같은 정상 흐름을 한 단계씩 실행한다. 모집 완료, 확정·선지급, 세 차례 정산, 만기와 수익 청구가 각각 화면에 반영되며 정산 단계에서는 Anvil 블록 시각만 다음 정산일로 이동한다. `처음 상태로 되돌리기`는 패널을 처음 연 시점의 Anvil snapshot을 복원한다. 이 제어 API는 개발 Compose와 체인 ID `31337` 조합에서만 활성화된다.
