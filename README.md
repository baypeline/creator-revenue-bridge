...
로컬 컨트랙트를 Anvil에 배포하고 역할, 모의 정산 토큰 잔액과 투자 가능한 데모 상품을 초기화하려면 다음 명령을 사용한다.
...
개발 화면 오른쪽 아래의 `데모 덱`은 같은 정상 흐름을 한 단계씩 실행한다. 모집 완료, 확정·선지급, 세 차례 정산, 만기와 수익 청구가 각각 화면에 반영되며 정산 단계에서는 Anvil 블록 시각만 다음 정산일로 이동한다. `새 컨트랙트로 초기화`는 Factory를 통해 새 mUSD·Bridge·수익권 토큰과 상품 세 건을 배포하고 화면의 활성 주소를 전환한다. 단계 제어 API는 개발 Compose와 체인 ID `31337` 조합에서만 활성화된다.
